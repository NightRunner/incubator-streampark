...
<script lang="ts" setup>
  ...
  import {
    fetchAddVariableCreateTable,
    fetchCheckVariableCode,
    fetchUpdateVariable,
    fetchVariableInfo,
  } from '/@/api/flink/variable/create/table';
  ...
  async function handleVariableCodeBlur(e) {
    ...
    if (value.length) {
      ...
      if (value.length < 3 || value.length > 50) {
        ...
        setHelp(t('flink.variableCreateTable.form.len'));
        return Promise.reject();
      } else if (!new RegExp(/^([A-Za-z])+([A-Za-z0-9._-])+$/).test(value)) {
        setValidateStatus('error');
        setHelp(t('flink.variableCreateTable.form.regExp'));
      } else {
        ...
        if (data.status !== 'success') {
          ...
          setValidateStatus('error');
          setHelp(t('flink.variableCreateTable.form.exists'));
          return Promise.reject();
        }
      }
    } else {
      setValidateStatus('error');
      setHelp(t('flink.variableCreateTable.form.empty'));
      return Promise.reject();
    }
  }
  ...
  const getTeamFormSchema = computed((): FormSchema[] => {
    return [
      {
        field: 'variableCode',
        label: t('flink.variableCreateTable.table.variableCode'),
        component: 'Input',
      ...
      {
        field: 'variableValue',
        label: t('flink.variableCreateTable.table.variableValue'),
        component: 'InputTextArea',
        componentProps: {
          rows: 12,
          placeholder: t('flink.variableCreateTable.table.variableValuePlaceholder'),
        },
        rules: [{ required: true, message: t('flink.variableCreateTable.table.variableValuePlaceholder') }],
      },
      {
        field: 'description',
  ...
  const getTitle = computed(() =>
    !unref(isUpdate) ? t('flink.variableCreateTable.addVariable') : t('flink.variableCreateTable.modifyVariable'),
  );
  // form submit
  async function handleSubmit() {
    try {
      ...
      await (isUpdate.value
        ...
        : fetchAddVariableCreateTable(values));
      closeDrawer();
