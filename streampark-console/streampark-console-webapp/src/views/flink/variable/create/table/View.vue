            ...
            :actions="[
              {
                ...
                tooltip: t('flink.variableCreateTable.modifyVariable'),
              ...
              },
              {
                ...
                tooltip: t('flink.variableCreateTable.table.depend'),
                auth: 'variable:depend_apps',
                onClick: () => router.push('/flink/variable/create/table/depend_apps?id=' + record.variableCode),
              },
              {
                icon: 'ant-design:delete-outlined',
                color: 'error',
                tooltip: t('flink.variableCreateTable.deleteVariable'),
                auth: 'variable:delete',
                popConfirm: {
                  title: t('flink.variableCreateTable.deletePopConfirm'),
...
<script lang="ts" setup>
  ...
  import { fetchVariableDelete, fetchVariableList } from '/@/api/flink/variable/create/table';
  ...
  const [registerTable, { reload }] = useTable({
    title: t('flink.variableCreateTable.table.title'),
  ...
  async function handleDelete(record: Recordable) {
    ...
    if (data.status === 'success') {
      createMessage.success(t('flink.variableCreateTable.deleteVariable') + t('flink.variableCreateTable.success'));
      reload();
    } else {
      createMessage.error(t('flink.variableCreateTable.deleteVariable') + t('flink.variableCreateTable.fail'));
    }
  }

  function handleSuccess(isUpdate: boolean) {
    createMessage.success(
      `${isUpdate ? t('common.edit') : t('flink.variableCreateTable.add')}${t('flink.variableCreateTable.success')}`,
    );
    reload();
  }
</script>
